"""
InsightFlow Azure OpenAI LLM Provider

使用 Azure OpenAI 生成洞察
"""
...
class AzureConfig:
    ...
    def from_env(cls) -> "AzureConfig":
        ...
        return cls(
            ...
            deployment_name=os.getenv("AZURE_OPENAI_API_DEPLOYMENT_NAME", "gpt-4o"),
            api_version=os.getenv("AZURE_OPENAI_API_VERSION", "2024-02-15-preview")
        )


class AzureLLMProvider:
    """Azure OpenAI LLM 提供者"""
...
- 使用中文回复"""
    ...
    def _build_user_prompt(
        ...
        if samples:
            prompt_parts.append(f"\n## 内容样本（最近 {len(samples)} 条）\n" +
                              "\n".join(samples[:10]))  # 限制样本数量
    ...
    def _parse_response(self, response: str, events_data: Dict[str, Any]) -> Insight:
        ...
        return Insight(
            ...
            metadata={"raw_response": response[:500]}  # 保留部分原始响应用于调试
        )
